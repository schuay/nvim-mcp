...
"""`showme install <harness>` writes one entry into a config file that belongs
to the human, and has to leave the rest of it alone."""
...
def test_every_harness_gets_an_entry_naming_this_showme(home: Path, key: str) -> None:
    harness = install.HARNESSES[key]
    path, shown, text = install.plan(harness, "/opt/showme")
    assert text is not None
    assert "/opt/showme" in shown
    install.apply(path, text)

    # And a second run is a no-op rather than a second entry.
    _, _, again = install.plan(harness, "/opt/showme")
...
def test_the_rest_of_a_config_survives(home: Path) -> None:
    ...
    path, _, text = install.plan(install.HARNESSES["gemini"], "/opt/showme")
    assert text is not None
    backup = install.apply(path, text)

    document = json.loads(settings.read_text())
    assert document["ui"] == {"theme": "dark"}
    assert sorted(document["mcpServers"]) == ["showme", "x"]
...
def test_a_toml_config_is_appended_to_not_rewritten(home: Path) -> None:
    ...
    path, _, text = install.plan(install.HARNESSES["codex"], "/opt/showme")
    assert text is not None
    install.apply(path, text)
    ...
    assert servers["showme"] == {"command": "/opt/showme", "args": ["mcp"]}
...
def test_a_config_this_cannot_parse_is_described_not_rewritten(home: Path) -> None:
    ...
    with pytest.raises(install.Unparsable):
        install.plan(install.HARNESSES["gemini"], "/opt/showme")
    assert settings.read_text() == before
    # There is still something to tell the human to paste.
    assert "showme" in install.snippet(install.HARNESSES["gemini"], "/opt/showme")
